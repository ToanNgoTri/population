...
console.log('searchRearch',searchRearch);

  function Item({item,index}) {
    return (
      <View
      style={{flexDirection:'collumn',backgroundColor: route.params.CCCD == item['CCCD'] ?'orange':index%2?'#CCCCCC':'#white'   ,marginTop:10, flexWrap:'wrap',
        padding:10,borderWidth:2,borderColor:'black',borderRadius:5
...
<View>
        ...
        <Text style={{marginRight:10}}>Quan hệ: {item['QUANHE']=='CH'?<Text style={{fontWeight:'bold',backgroundColor:'yellow'}}>{item['QUANHE']}</Text>:item['QUANHE']}</Text>
        <Text style={{marginRight:10}}>Họ và tên: <Text style={{fontWeight:'bold'}}>{item['HOTEN']}</Text></Text>

        <Text style={{marginRight:10}}>Ngày sinh: {item['NAMSINH']}</Text>

        <Text style={{marginRight:10}}>Giới tính: {item['GIOITINH']=="TRUE"?'Nam':'Nữ'}</Text>
  ...
  return (
    <>
      <View style={{paddingLeft:20,paddingRight:20,marginBottom:20,marginTop:10}}>
        <View style={{justifyContent:'space-between',flexDirection:'row'}}>
          <Text style={{fontWeight:'bold'}}>
            Số HSHK {route.params.screen}
          </Text>
          <Text style={{fontWeight:'bold'}}>
          Địa chỉ: {searchRearch[0] && searchRearch[0]['NOITHTRU']}          
          </Text>
        </View>
        <FlatList
          data={searchRearch}
          renderItem={item => <Item item={item.item} index={item.index}/>}></FlatList>
      </View>
    </>
  );
}
